...
#define BMS_MAC_ADDRESS "AA:C2:37:0A:B0:DC" 
...
void decoderErreursJBD(uint16_t protStatus) {
  if (protStatus == 0) {
    Serial.println("Batterie 2 / Statut Protection : RAS (Aucune erreur, tout est normal)");
    return;
  }
  
  Serial.print("⚠️ ALERTE / PROTECTION(S) ACTIVÉE(S) : ");
  if (protStatus & (1 << 0))  Serial.print("[Batterie 2 / Surtension bloc cellule] ");
  if (protStatus & (1 << 1))  Serial.print("[Batterie 2 / Sous-tension bloc cellule] ");
  if (protStatus & (1 << 2))  Serial.print("[Batterie 2 / Surtension pack global] ");
  if (protStatus & (1 << 3))  Serial.print("[Batterie 2 / Sous-tension pack global] ");
  if (protStatus & (1 << 4))  Serial.print("[Batterie 2 / Surchauffe en charge] ");
  if (protStatus & (1 << 5))  Serial.print("[Batterie 2 / Sous-température en charge] ");
  if (protStatus & (1 << 6))  Serial.print("[Batterie 2 / Surchauffe en décharge] ");
  if (protStatus & (1 << 7))  Serial.print("[Batterie 2 / Sous-température en décharge] ");
  if (protStatus & (1 << 8))  Serial.print("[Batterie 2 / Surintensité / Surcharge] ");
  if (protStatus & (1 << 9))  Serial.print("[Batterie 2 / Surintensité en décharge] ");
  if (protStatus & (1 << 10)) Serial.print("[Batterie 2 / Court-Circuit !] ");
  if (protStatus & (1 << 11)) Serial.print("[Batterie 2 / Erreur IC de gestion] ");
  if (protStatus & (1 << 12)) Serial.print("[Batterie 2 / Erreur MOSFET] ");
  Serial.println();
}
...
void decoderTrameJBD(uint8_t* buffer, size_t taille) {
  ...
  if (typeTrame == 0x03) {
    ...
    Serial.print("Batterie 2 / Tension Totale     : "); Serial.print(tensionVolts); Serial.println(" V");

    int16_t courantBrut = (buffer[6] << 8) | buffer[7];
    float courantAmperes = courantBrut / 100.0;
    Serial.print("Batterie 2 / Courant / Intensité : "); Serial.print(courantAmperes); Serial.println(" A");

    float capRestanteAh = ((buffer[8] << 8) | buffer[9]) / 100.0;
    Serial.print("Batterie 2 / Capacité Restante   : "); Serial.print(capRestanteAh); Serial.println(" Ah");

    Serial.print("Batterie 2 / Pourcentage (SoC)   : "); Serial.print(buffer[23]); Serial.println(" %");
    Serial.print("Batterie 2 / Nombre de Cellules  : "); Serial.println(buffer[25]);
    ...
    for (int i = 0; i < nbCellulesTrame; i++) {
      ...
      Serial.print("Batterie 2 / Cellule "); 
...
bool connecterAuBMS(String targetAddress) {
  Serial.print("Batterie 2 / Connexion à l'adresse MAC : ");
  ...
  if (!pClient->connect(bmsAddress)) {
    Serial.println("Batterie 2 /  -> Échec.");
    return false;
  }
  Serial.println("Batterie 2 /  -> Connecté !");
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("Batterie 2 / Démarrage du système JBD Monitor...");
  BLEDevice::init("");
...
void loop() {
  if (connecte && pClient->isConnected()) {
    ...
    Serial.println("Batterie 2 / BMS déconnecté. Reconnexion dans 10s...");
    delay(10000);
    connecte = connecterAuBMS(BMS_MAC_ADDRESS);
  }
